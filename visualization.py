...
def visualize_outlier_generation_quality(model, concated_input_features, adj, sample_normal_idx, 
    ...
    model.eval()
    with torch.no_grad():
        # Get embeddings and generated outliers
        train_flag = False
        ...
        all_embeddings = emb.squeeze(0).cpu().numpy()  # [num_nodes, hidden_dim]
        ...
        # Get embeddings for different node types
        normal_embeddings = all_embeddings[real_normal_idx]
        real_anomaly_embeddings = all_embeddings[real_anomaly_idx]
        
        # Generate outliers using the model in training mode
        train_flag = True
        emb_train, emb_combine_train, logits_train, emb_con_train, emb_abnormal_train, con_loss_train, gui_loss_train = model(
            concated_input_features, adj.to(args.device), sample_normal_idx, 
            all_labeled_normal_idx, community_H, train_flag, args
        )
        
        generated_outliers = emb_con_train.cpu().numpy()  # [num_generated, hidden_dim]
        
        print(f"Normal nodes: {len(real_normal_idx)}")
        print(f"Real anomalies: {len(real_anomaly_idx)}")
        print(f"Generated outliers: {generated_outliers.shape[0]}")
        
        # Create comprehensive visualizations
        create_embedding_visualizations(
            normal_embeddings, real_anomaly_embeddings, generated_outliers,
...
def load_best_model_and_visualize(args, model, concated_input_features, adj, sample_normal_idx, 
    ...
    print(f"Using best model from epoch {records.get('best_test_auc_epoch', 'unknown')}")
